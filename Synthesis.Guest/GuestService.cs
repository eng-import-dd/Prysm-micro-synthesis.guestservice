...
namespace Synthesis.GuestService
{
    ...
    internal sealed class GuestService : StatelessService
    {
        ...
        {
            var loggerFactory = GuestServiceBootstrapper.RootContainer.Resolve<ILoggerFactory>();
        ...
        private async Task ReportHealth(ServiceContext context, CancellationToken cancellationToken)
        {
            ...
            while (!cancellationToken.IsCancellationRequested)
            {
                ...
                try
                {
                    ...
                    {
                        var info = new HealthInformation(context.NodeContext.NodeName, "Guest-healthCheck", HealthState.Error)
                        {
                    ...
                    {
                        var info = new HealthInformation(context.NodeContext.NodeName, "Guest-healthCheck", HealthState.Ok);
                        Partition.ReportInstanceHealth(info);
                    }
                }
